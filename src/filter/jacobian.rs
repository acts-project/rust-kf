...
pub fn linear(
    ...
    let loc_2_glob = local_to_global_jac(&angles);
    let glob_2_loc = global_to_local_jac(&angles);
    let transport_jac = linear_transport_jac(&mut angles, distance);


    return loc_2_glob * transport_jac * glob_2_loc
}
...
fn global_to_local_jac(
    ...
    let mut global_to_local_jacobian = Mat8x5::zeros();

    // add values into transport jacobian
    change_mat_val!{
        global_to_local_jacobian: 5;
...
fn linear_transport_jac(
    ...
    change_mat_val!{transport_jac: 8;
